...
def get_ddi_mask_recap(atc42SMLES, med_voc):
    ...
    for k, v in med_voc.idx2word.items():
        ...
        for SMILES in atc42SMLES[v]:
            try:
                m = Recap.RecapDecompose(Chem.MolFromSmiles(SMILES))
                for frac in sorted(m.children.keys()):
                    tempF.add(frac)
            except:
                pass
        fraction.append(tempF)
...
if __name__ == '__main__':
    ...
    med_file = '../data/input/PRESCRIPTIONS.csv'
    diag_file = '../data/input/DIAGNOSES_ICD.csv'
    procedure_file = '../data/input/PROCEDURES_ICD.csv'

    # input auxiliary files
    # med_structure_file = './output/atc32SMILES.pkl'
    RXCUI2atc4_file = '../data/input/ndc2atc_level4.csv' 
    cid2atc6_file = '../data/input/drug-atc.csv'
    rxnorm2RXCUI_file = '../data/input/ndc2rxnorm_mapping.txt'
    ddi_file = '../data/input/drug-DDI.csv'
    drugbankinfo = '../data/input/drugbank_drugs_info.csv'

    # output files
    ddi_adjacency_file = "../data/output/ddi_A_final.pkl"
    ehr_adjacency_file = "../data/output/ehr_adj_final.pkl"
    ehr_sequence_file = "../data/output/records_final.pkl"
    vocabulary_file = "../data/output/voc_final.pkl"
    ddi_mask_H_file = "../data/output/ddi_mask_H.pkl"
    atc3toSMILES_file = '../data/output/atc3toSMILES.pkl'

    ddi_mask_H_recap_file = "../data/output/ddi_mask_H_recap.pkl"
    ...
    # get ddi_make_H_recap
    ddi_mask_H_recap = get_ddi_mask_recap(atc3toSMILES, med_voc)
    dill.dump(ddi_mask_H_recap, open(ddi_mask_H_recap_file, 'wb'))    
